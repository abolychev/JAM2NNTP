...
sub nntpd_cmd_post {
  my ($kernel,$sender,$client_id) = @_[KERNEL,SENDER,ARG0];
  $kernel->post( $sender, 'send_to_client', $client_id, '340 send article to be posted. End with <CR-LF>.<CR-LF>' );
  return;
}

sub nntpd_posting {
  my ($kernel,$sender,$client_id, $text) = @_[KERNEL,SENDER,ARG0,ARG2];
  my @subfields = ();
  print Dumper($text);
...
#  push @subfields, 'TZUTC: 0300'; 
  ...
  my $headerref = {};
  $headerref->{DateProcessed} = $headerref->{DateReceived} = $headerref->{DateWritten} = FTN::JAM::TimeToLocal(time);
  ...
  $kernel->post( $sender, 'send_to_client', $client_id, '240 article posted ok' );
  return;
}
...
#	  print "CLOSE $handle " . $_[HEAP]->{clients}->{ $client_id }->{group} . "\n";
  ...
  $handle = FTN::JAM::OpenMB( $groups{$group} ) or die( "OpenMB : $group : $groups{$group}" );
  FTN::JAM::GetMBSize($handle,\$num) or die;
  $_[HEAP]->{clients}->{ $client_id } = { group => $group };
  $_[HEAP]->{clients}->{ $client_id }->{handle} = $handle;
  $kernel->post( $sender, 'send_to_client', $client_id, "211 $num 1 $num $group selected" );
  return;
}
...
sub common_article {
  my $params=shift;
  my $command=shift;
  my ($kernel,$sender,$client_id,$article) = @{$params}[KERNEL,SENDER,ARG0,ARG1];


  #print Dumper($groups{$group});
  
  if ( !$article or !defined ${$params}[HEAP]->{clients}->{ $client_id }->{group} ) {
...
#  print "ARTICLE group: $group client: $client_id article: $article handle: $handle\n";  
  ...
  my(%header,@subfields,$text);
  my $success = FTN::JAM::ReadMessage($handle, $article,\%header,\@subfields,\$text);
  ...
  my $msg_id = $article;
	...
	if( $command eq 'head' ) {
#		print "head\n";
		$kernel->post( $sender, 'send_to_client', $client_id, "221 $msg_id <0> article retrieved - head follow" );
		$textsend = message_header($group, \%header, \@subfields);
	} elsif ( $command eq 'article') {
#		print "article\n";
		$kernel->post( $sender, 'send_to_client', $client_id, "220 $msg_id <0> article retrieved - head and body follow" );
		$textsend = message_header($group, \%header, \@subfields);
		$textsend .= "\n";
		$text =~ s/\r/\n/g;
		$textsend .= $text;
...
#	print Dumper($subfields);
	...
	my $dt = DateTime->from_epoch( epoch => FTN::JAM::LocalToTime( $header->{DateWritten} ) );
...
"Subject: $subj\n" .
"From: " . $fields{2} . " $faddr\n" .
"Message-ID: <" . $group . '+' . $msgid . "\@localhost>\n" .
( $refer ? "References: <" . $group . '+' . $refer . "\@localhost>\n" : '' ) .
"Content-Type: text/plain; charset=CP866\n" .
"X-Comment-To: " . $fields{3} . "\n";
